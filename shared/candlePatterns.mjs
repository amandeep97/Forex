// shared/candlePatterns.mjs
// Candlestick pattern detection for OHLC candle arrays.
// Each candle must have { o, h, l, c } fields.
//
// In shared/ rather than src/utils because four things now read it and they
// must not drift: the Screener, the alerts engine, the live bot (which imports
// this file by path), and the regime study, which is the only one of them that
// asks whether any of these patterns is worth anything. The two localStorage
// helpers stay in src/utils/candlePatterns.js, which re-exports everything
// here — the bot has no localStorage and does not need one.
...
// back inside it — stops grabbed on the whole range, then reversed. Higher
// quality than a lone hammer or star because it must sweep the full range, not
// just poke a low. Whether "higher quality" survives a holdout is exactly what
// the regime study is now being asked; until it answers, that word is a claim.

export const DEFAULT_PATTERN_N = 5
